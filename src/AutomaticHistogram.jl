...
- `dist`: The name of the distance to compute. Valid options are `:iae` (default), `:ise`, `:hell`, `:sup`, `:kl`, `:lp`. For the ``L_p``-metric, a given power `p` can be specified as a keyword argument. 
