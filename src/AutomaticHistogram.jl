...
- `dist`: The name of the distance to compute. Valid options are `:iae` (default), `:ise`, `:hellinger`, `:sup`, `:lp`. For the ``l_p``-metric, a given power `p` can be specified as a keyword argument. 
